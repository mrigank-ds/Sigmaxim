  ...
  protected function addStateToGroup(array $new_states, array $options, array $states) {
    ...
    foreach ($new_states as $key => $constraints) {
      if (empty($states[$key][$options['grouping']])) {
        ...
        // yugasa changes
         array_push($states[$key][$options['grouping']], $constraints);
      }
    }
  ...
  protected static function evaluateDependency($context, $values, array $options) {
    if ($options['values_set'] == ConditionalFieldsInterface::CONDITIONAL_FIELDS_DEPENDENCY_VALUES_WIDGET) {
      ...
      if ($options['condition'] === '!empty') {
        if (is_array($values)) {
          $values = (isset($values[0]['wrapper']['target_id'])) ? $values[0]['wrapper']['target_id'] : (isset($values[0]['value']) ? $values[0]['value'] : $values);
        }
      ...
      if ($context == 'edit') {
        // Handle wrapper-based widgets (Data Reference / Advanced Data Reference).
        if (is_array($values) && isset($values[0]['wrapper']['target_id'])) {
          $values = [['target_id' => $values[0]['wrapper']['target_id']]];
        }
