...
class ConditionalFieldEditForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $entity_type = NULL, $bundle = NULL, $field_name = NULL, $uuid = NULL) {
    ...
    $form['value'] = [
      ...
      '#tree' => FALSE,
      $condition['dependee'] => $dummy_field,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($field_names as $field_name) {
      ...
      // Set field value.
      if (isset($settings[$dependee])) {
        // Get and save value as string.
        $value = $settings[$dependee];

        // Handle datetime objects.
        if (is_array($value) && !empty($value[0]['value']) && is_object($value[0]['value']) && $value[0]['value'] instanceof DrupalDateTime) {
          foreach ($value as $delta => $date) {
            if (!empty($date['value']) && $date['value'] instanceof DrupalDateTime) {
              $value[$delta]['value'] = $date['value']->format(DateTimeItemInterface::DATETIME_STORAGE_FORMAT);
            }
          }
        }

        // For complex widgets (e.g. Data Reference, Advanced Data Reference)
        // the submitted value is a nested array.
        // We normalize it to [target_id => ID] for comparison and JS states,
        // but save the raw version to restore the widget state on re-edit.
  ...
  protected function getDummyField($entity_type, $bundle, $condition, FormStateInterface $form_state, $default_value = NULL) {
    ...
    $default_value = $condition['settings']['value_form_raw'] ?? $condition['settings']['value_form'] ?? $default_value;

    // Populate entity target_id if we have one.
    if ($default_value) {
      $id = NULL;
      if (isset($default_value[0]['wrapper']['target_id'])) {
        $id = $default_value[0]['wrapper']['target_id'];
      }
      elseif (isset($default_value[0]['target_id'])) {
         // Could be a simplified ID or comma-separated.
         $id = $default_value[0]['target_id'];
      }

      if ($id) {
        $dummy_entity->set($field_name, [['target_id' => $id]]);
      }
      else {
    ...
    // Do NOT set user input from the saved default_value here. Doing so causes
    // drill-down widgets (e.g. sigmaxim_simple_data_reference) to filter each
    // select's options to only the saved value, because renderResultingEntity()
    // reads $form_state->getValue() to determine which stage IDs to pass to
    // getStageOptions(). With user input set, every subsequent level in the
    // drill-down is filtered by the selected value of the previous level,
    // leaving only the saved option. Without user input, $form_state->getValue()
    // returns NULL for the wrapper and getStageOptions() receives all entity IDs,
    // so all available options are shown. The correct selected value is still
    // displayed because updateOptions() sets #default_value on each select from
    // the referenced entity loaded via $item->entity.
